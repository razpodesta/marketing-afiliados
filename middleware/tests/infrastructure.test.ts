...
describe("Protocolo de Validación: Manejadores de Infraestructura y UX", () => {
    ...
    vi.mocked(createSupabaseMiddlewareClient).mockImplementation(
      (request: NextRequest) => {
        ...
        const mockSupabase: SupabaseClient<Database> = {
          from: vi.fn((tableName: string) => { // CORRECCIÓN (TS2554): 'from' debe aceptar tableName
            return {
              select: vi.fn().mockReturnThis(),
              eq: vi.fn((column: string, value: any) => { // CORRECCIÓN (TS2554): 'eq' debe aceptar column y value
                return {
                  single: vi.fn().mockImplementation(() => {
                    ...
                    const normalizedQuerySubdomain = (value as string).toLowerCase(); // Asegurar que el valor es string

                    if (normalizedQuerySubdomain === "cliente-alfa") {
                      return Promise.resolve({ data: { id: "site-123", subdomain: "cliente-alfa" }, error: null });
                    }
                    if (normalizedQuerySubdomain === "cliente-beta") {
                      return Promise.resolve({ data: { id: "site-456", subdomain: "cliente-beta" }, error: null });
                    }
  ...
  describe("Suite 6: Multi-Tenancy (handleMultitenancy)", () => {
    it("6.1: Debe reescribir la URL a la ruta de subdominio correcta cuando el host es un subdominio válido", async () => {
      ...
      const { supabase: mockSupabase } = await createSupabaseMiddlewareClient(request);
...
==================== FIN DEL ARCHIVO [middleware/tests/infrastructure.extended.test.ts] ====================


==================== INICIO DEL ARCHIVO [middleware/tests/infrastructure.test.ts] ====================
...
==================== FIN DEL ARCHIVO [middleware/tests/infrastructure.test.ts] ====================

--- START OF FILE 2025-08-01-01-24-31_marketing-afiliados-snapshot.txt ---